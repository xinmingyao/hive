...
local encode_base64 = require "base64"
...
function _M.connect(self, uri, opts)
    ...
    local ok, err
    if sock_opts then
        socket = cell.connect(host, port, sock_opts)
    else
        socket = cell.connect(host, port)
    end
    if not socket then
    ...
    local key = encode_base64.encode(bytes)
   ...
   -- end
    ...
    local header_reader = socket:readline("\r\n\r\n")
